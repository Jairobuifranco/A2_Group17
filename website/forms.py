...
class EventForm(FlaskForm):
    ...
    name = StringField('Event Name', validators=[InputRequired('Please enter an event name'), Length(max=25)])
    ...
    price = DecimalField('Price', validators=[InputRequired('Please enter the ticket price'), NumberRange(min=0, message='Price can not be negative')])
    ticket_quantity = IntegerField('Ticket Quantity', validators=[InputRequired('Please enter the ticket quantity'), NumberRange(min=1, max=5000, message='Quantity must be between 1 and 5000')])
    capacity = IntegerField('Capacity', validators=[InputRequired('Please enter the capacity'), NumberRange(min=1, max=10000, message='Capacity must be between 1 and 10,000')])
    ...
    # Define a function to validate capacity cannot be less than ticket quantity #
    def validate_capacity(self, field):
        if (field.data and hasattr(self, 'ticket_quantity') and self.ticket_quantity.data and field.data < self.ticket_quantity.data):
            raise ValidationError("Capacity cannot be less than ticket quantity")
